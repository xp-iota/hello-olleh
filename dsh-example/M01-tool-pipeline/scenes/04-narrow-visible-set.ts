/**
 * 模块 M01 的对应场景：注册 4 个工具 → 插件在 `agent/session-start` 时只放行 echo/read →
 ...
 */
import { REAL_PROVIDER, createHarness, demoTool, realConfig } from '../../runtime/harness.ts'
import * as restrictPlugin from '../impl/04-tool-restrict.ts'
...
const second = await ctx.agents.create({ sessionId: 'agent-2' as any, agentOptions: { provider: REAL_PROVIDER, model: realConfig().model } })
