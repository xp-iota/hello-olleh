 ...
 *
 * 与 M01.2 的 `tools/pre-execute` 相比：pre-execute 是一串可重排的策略监听者，后面的
 * 监听者可以推翻前面的结论；而 `guard()` 注册的是 **pre-execute 之后的单调拒绝**——
 * guard 只能拒绝、没有"放行"这个返回值，所以监听器顺序无法把已有拒绝改回放行。
...
// `name` 只是插件/Fiber 标识；拒绝规则由 apply 中的 tools.guard 注册。
