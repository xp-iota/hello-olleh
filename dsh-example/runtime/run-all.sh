#!/usr/bin/env bash
# 依次运行全部 12 个离线方向模块。从 dsh-example 根目录调用：npm run all
# 真实 provider 路径 M01:real / M03:real / M10:real 不在此脚本中，避免隐式网络请求。
set -euo pipefail
cd "$(dirname "$0")/.."
...
for module in "${modules[@]}"; do
  ...
  echo "  ▶ 运行模块 ${module}"
  echo "══════════════════════════════════════════════════════════════"
  "$NODE_BIN" "$module/run.ts"
done

echo ""
echo "✅ 全部 12 个离线方向模块运行完毕。"
