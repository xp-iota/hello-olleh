/**
 * 模块 M09 的对应场景：演示任务启动准入、JobHooks 拉模型、增量读、wait、kill 与属主栅栏。`npm run M09`。
 */
import type { JobOutcome } from '@deepseek-ai/dsh-jobs'
import { createHarness, REAL_PROVIDER, realConfig } from '../../runtime/harness.ts'
import * as plugin from '../impl/01-jobs-background.ts'
...
console.log('\n④ 属主栅栏：非属主读取被拒:')
const other = await ctx.agents.create({ sessionId: 'other-agent' as any, agentOptions: { provider: REAL_PROVIDER, model: realConfig().model } })
try {
