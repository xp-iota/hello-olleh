 ...
 * 在这里被直接演示：差异只有 llm.stream() 的 `provider` 从 'mock' 换成 'minimax-m3'，
 * 后者是 harness 检测到 MINIMAX_API_KEY 时注册的真实适配器（见 runtime/llm-minimax.ts，
 * MiniMax 的 Anthropic 兼容端点，真实 HTTP + SSE）。
 *
 * mock 路由的离线消费与逐 chunk 协议追踪集中在 run.ts（离线套件只跑它）；本脚本
 * 只消费 `minimax-m3` 路由，与 run.ts 的 mock 段共用同一组协议检查断言。
 *
...
if (!process.env.MINIMAX_API_KEY) {
  ...
  console.error('  MINIMAX_API_KEY=<your-key> npm run M03:real')
  process.exit(1)
}

const model = process.env.MINIMAX_MODEL ?? 'MiniMax-M3'
// 'mock' 路由由 harness 内置适配器占位（本插件如何在 'mock' 路由上注册与被消费，见 run.ts）；
// 'minimax-m3' 路由由 harness 检测到密钥后注册 —— 本脚本只消费真实路由，不需要示例插件。
const harness = await createHarness()

console.log('① 已注册的 provider 路由:', harness.ctx.llm.listProviders().map((info) => info.id))
console.log('② 目标模型:', `minimax-m3 / ${model}`)

// —— 消费 minimax-m3 路由（真实 HTTP + SSE）——
// 消费代码与 run.ts 的 mock 段共用同一个 consumeStream()，只有 stream() 的入参不同。
console.log('\n③ 消费 minimax-m3 路由（真实 HTTP + SSE）:', '（mock 路由的消费见 run.ts）')
...
if (!real.finish || real.usage === undefined || real.text === '' || real.orphanDeltas > 0) {
  throw new Error('minimax-m3 流未守住 StreamChunk 协议契约 —— LLM seam 未被验证')
}
