/**
 * 模块 M11 的对应阶段：默认值 → update 合并 → expectedRevision 冲突 → mutate 路径编辑 → replace 重置。`npm run M11`。
 */
import { createHarness } from '../../runtime/harness.ts'
import * as plugin from '../steps/01-settings-namespaces.ts'
import { DEMO_NS, DemoSchema } from '../steps/01-settings-namespaces.ts'
