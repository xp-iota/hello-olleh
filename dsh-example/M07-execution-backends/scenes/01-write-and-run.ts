/**
 * 模块 M07 的对应场景：消费真实 ctx.fs / ctx.subprocess seam 做真实副作用，并演示 exec.signal 取消。
 * `npm run M07`。
 *
...
import * as fsShellPlugin from '../impl/01-fs-shell-side-effects.ts'
...
console.log('    注意 code = ABORTED_BEFORE_DISPATCH：impl/01-fs-shell-side-effects.ts 里那句 `signal.aborted` 守卫')
