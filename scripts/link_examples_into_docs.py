...
> `ctx.shell` → [M07.2]({EX}/M07-execution-backends/phases/02-run-and-start.ts)、`ctx.sandbox` → [M07.3]({EX}/M07-execution-backends/steps/03-sandbox-seam.ts)、
...
> `ctx.jobs` → [M09.1]({EX}/M09-long-running-orchestration/phases/01-job-start-read-kill.ts)、`ctx.goals` → [M09.2]({EX}/M09-long-running-orchestration/phases/02-goal-cas-conflict.ts)、
...
> （一个 `SKILL.md`，零行 TypeScript）；它的 [`run.ts`]({EX}/M10-external-capabilities/phases/01-inject-skill-into-turn.ts) 把这份 Markdown
...
> [`M12.2 · dispose-pending-timer`]({EX}/M12-framework-mechanisms/phases/02-dispose-pending-timer.ts) 演示 effect 化的定时器与 fiber 卸载时的拒绝语义。
"""),
...
> {MARK}：[`M04.1 · observe-turn-events`]({EX}/M04-agent-loop-intervention/phases/01-observe-turn-events.ts) 用真实 agent-loop 跑一个 turn，
...
> {MARK}：[`M04.3 · compare-inbox-channels`]({EX}/M04-agent-loop-intervention/phases/03-compare-inbox-channels.ts) 把四个入口各调一遍，
> 并把会话日志里的 `agent/inbox/spliced` 流水打出来 —— "谁在什么时候塞了什么"是可回放的。
"""),
    ('06-Agent循环与会话日志.md', '6.12', f"""
> {MARK}：[`M05.1 · check-log-invariants`]({EX}/M05-session-surface/phases/01-check-log-invariants.ts) 逐条验证本节的不变量：
> seq 连续、事件深冻结（改写抛 `TypeError`）、`deriveMessages()` 只投影三类消息事件。
"""),
    ('06-Agent循环与会话日志.md', '6.15', f"""
> {MARK}：[`M05.1 · check-log-invariants`]({EX}/M05-session-surface/phases/01-check-log-invariants.ts) 用真实 `replace` 折叠了两个节点并打印
...
> {MARK}：[`M05.1 · check-log-invariants`]({EX}/M05-session-surface/phases/01-check-log-invariants.ts) 第 ⑥ 节演示 fork 的稳定前缀要求 ——
...
> {MARK}：[`M07.2 · run-and-start`]({EX}/M07-execution-backends/phases/02-run-and-start.ts) 把三个方法都跑了一遍 ——
...
> {MARK}：[`M02.3 · fold-history`]({EX}/M02-context-assembly-economics/phases/03-fold-history.ts) 实测把 12 个 surface 节点折成 4 个
...
> {MARK}：[`M11.1 · settings-cas-conflict`]({EX}/M11-config-data-infrastructure/phases/01-settings-cas-conflict.ts) 走完
...
> {MARK}：[`M09.1 · job-start-read-kill`]({EX}/M09-long-running-orchestration/phases/01-job-start-read-kill.ts) 覆盖 JobHooks 拉模型、增量读、
> `wait` 结算、属主栅栏、`kill`，以及**没有 controller 时 start 被拒**的准入。
> 注意 `JobKind` 是封闭词表（`'bash' | 'subagent'`），`label` 必填。
"""),
    ('09-宿主与运行面-Web网关编排存储类型.md', '9.23', f"""
> {MARK}：[`M09.2 · goal-cas-conflict`]({EX}/M09-long-running-orchestration/phases/02-goal-cas-conflict.ts) 走完
