---
title: "Streaming 模式下推理服务异常时的 Agent 自保机制分析"
---
...
- `sources/opencode/packages/core/src/session/runner/llm.ts`
- `sources/opencode/packages/core/src/session/runner/retry.ts`
- `sources/opencode/packages/core/src/session/runner/step.ts`
- `sources/opencode/packages/core/src/session/compaction.ts`
- `sources/opencode/packages/core/src/session/message.ts`
...
![设计自己的 streaming agent：自保清单](diagrams/03-streaming-agent-resilience-streaming-agent.svg)

**设计自己的 streaming agent：自保清单** — [交互版](diagrams/03-streaming-agent-resilience-streaming-agent.html)（明暗主题 / 缩放 / 关系追踪 / 导出） · [IR 源](diagrams/03-streaming-agent-resilience-streaming-agent.architecture.json)

- **组成**：画布 16 个节点 · 源图共 39 个节点，其余见正文
- **关系**：源图 18 条有向关系 · 画布按主链顺序排列，完整关系见下方要点与正文
- **要点**：用户请求进入 → 同一 session 是否已有 tur… · 同一 session 是否已有 tur… → 排队或拒绝 / per-session…（是） · 同一 session 是否已有 tur… → 建立 turn 状态 / attempt=…（否）
...
| OpenCode | `sources/opencode/packages/core/src/session/runner/retry.ts` | `retry-after-ms` / `retry-after` |
| OpenCode | `sources/opencode/packages/core/src/session/runner/step.ts` | stream/retry/error/tool part processing |
| OpenCode | `sources/opencode/packages/core/src/session/compaction.ts` | context overflow pre-check |
