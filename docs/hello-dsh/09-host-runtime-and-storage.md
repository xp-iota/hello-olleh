---
title: "宿主与运行面：Web、网关、编排、存储、类型"
---
# 宿主与运行面：Web、网关、编排、存储、类型
...
> **路径缩写**：`P/` = `packages/`，`CL/` = `packages/client/`，`TP/` = `packages/typert/protocol/src/`。行号对应快照 `fb2c4b9e`。
>
...
💡 **`client-schema-form` 只有 195 行**却很关键：DSH 的插件配置都有 schemastery schema（[09 篇](09-host-runtime-and-storage.md)），前端**自动生成配置表单**。所以加一个插件不需要写设置界面。

💡 **`client-hmr` 与 cordis 的 hmr 是两套**：前者是浏览器侧（Vite HMR 集成），后者是 Node 侧模块热替换（[cordis 08](../hello-cordis/08-hmr.md)）。`web-app` bundle 里把 cordis 的 hmr 关掉了（`disabled: true`，[05 § 5.3](05-startup-and-cordis-runtime.md)）。
...
💡 **`isAppendSurfaceEvent(event)` 的使用**：只处理 `surfaceOp: 'append'` 的 `tool/result`，**跳过压缩产生的 `replace` 节点**（[06 § 6.15](06-agent-loop-and-session-log.md)）。前端与后端共享同一套表面语义。
...
`packages/extensions/cordis-client-runner`（5080 行，[04 § 4.4](04-extensions-and-ecosystem.md)）。
...
💡 **`web-app` bundle 的 patch 有 424 行**（[05 § 5.3](05-startup-and-cordis-runtime.md)），大部分是配置这些前端相关的行。

## 9.7 前端相关的工程约束

`scripts/` 里有专门的前端校验（[02 § 2.8](02-codebase-map.md)）：
...
💡 **`seq` 是断点续传的关键**：session 事件的 `seq` 就是日志数组下标（[06 § 6.14](06-agent-loop-and-session-log.md) ⑥）。客户端重连时报"我看到 seq N"，服务端从 N+1 继续推。**这是 append-only 设计的又一处红利。**

## 9.12 `api/gateway` 与 TypeRT

`packages/api/`（1807 行 src）：`gateway`（1358）+ `remotes`（449）。

💡 **`dsh-base` patch 里这一行**（[05 § 5.11](05-startup-and-cordis-runtime.md)）：
...
**id 叫 `typert-gateway` 但包名是 `dsh-api-gateway`** —— 说明网关的接口是**由 TypeRT 的类型注册表驱动的**（[09 篇](09-host-runtime-and-storage.md)）。
...
配合 `client/ui-settings-plugin-inventory` 与 `dsh plugin` 命令（[04 § 4.5](04-extensions-and-ecosystem.md)）。
...
| **ACP** | `acp`（532 行） | **自动化** | "automation-only"，权限是机器策略通道（[04 § 4.13](04-extensions-and-ecosystem.md)） |
| **DSH SDK** | `sdk/{client,protocol,server}`（4471 行） | **DSH↔DSH / 多语言** | 含 Python SDK（[04 篇](04-extensions-and-ecosystem.md)） |

💡 **三者都可以驱动同一个 agent**，因为它们都最终调 `ctx.agents`（[06 § 6.10](06-agent-loop-and-session-log.md)）。这是"everything is a plugin"在接口层的体现——**接入方式也是插件**。

## 9.17 `settings` 组

> 📐 **配套可跑示例**：[`M11.1 · settings-cas-conflict`](../../dsh-example/M11-config-data-infrastructure/scenes/01-settings-cas-conflict.ts) 走完
...
💡 **两者会交叉**：[06 § 6.11](06-agent-loop-and-session-log.md) 提到 `maxParallelToolCalls` 同时出现在插件 Config 与 Settings 里，且有 `source().maxParallelToolCalls`（`AL/index.ts:332`）——**settings 可覆盖启动配置**。前端有 `ui-settings` 系列 5 个包。
...
> 📐 **配套可跑示例**：[`M09.1 · job-start-read-kill`](../../dsh-example/M09-long-running-orchestration/scenes/01-job-start-read-kill.ts) 覆盖 JobHooks 拉模型、增量读、
...
💡 **`dsh-base` patch 里 jobs 那行直接用 `jobs-local`**（[05 § 5.11](05-startup-and-cordis-runtime.md)）：
...
💡 **`workflow-worker-thread` 比 definition 大 3.8 倍**：worker thread 的通信（序列化、生命周期、错误传播）是主要复杂度。与 `code-runtime-worker-thread`（[08 § 8.15](08-execution-services.md)）同一思路。
...
> 📐 **配套可跑示例**：[`M09.2 · goal-cas-conflict`](../../dsh-example/M09-long-running-orchestration/scenes/02-goal-cas-conflict.ts) 走完
...
💡 **`goal-round-driver` 是 `agent/turn-stopping` 最典型的用户**。[06 § 6.4](06-agent-loop-and-session-log.md) 提到 turn 里有**两次** `turnEnds && nextStep.length === 0` 检查（`AL/agent.ts:295` 与 `:299`），中间夹着 serial 派发——这个窗口就是给 goal 这类推进器用的。

💡 **与 subagent 的区别**（[08 § 8.31](08-execution-services.md)）：goal 在**同一会话**里推进（历史连续），subagent 派生**新会话**（上下文隔离）。
...
💡 **`todo/write` 是 13 个核心 session 事件之一**（`S/types.ts:299`，[06 § 6.13](06-agent-loop-and-session-log.md)）。它**不上表面**（不是 `SurfaceEventType`），所以：
- 不直接进模型历史
- 但持久、可回放、UI 能渲染
...
`client/ui-deliverables`（[09 § 9.3](09-host-runtime-and-storage.md)）的 `deliverablesDefinition` 累积"本 turn 产出了哪些文件"。
...
💡 **`storage-domain`（857 行）比两个 provider 都大**。"domain" 指的应该是**命名空间隔离**：每个插件有自己的存储域，互不干扰。这类似 cordis 的 isolate realm（[cordis 04 § 4.5](../hello-cordis/04-context-and-reflect-proxy.md)），但作用于持久数据。
...
💡 **与 compaction 的分工**（[08 § 8.37](08-execution-services.md)）：
- **spill**：内容还在，只是不在上下文里（可按需取回）
- **compaction**：内容被摘要替代（原文在日志里但不再进模型）

💡 **与 `util/output-retention`（[04 § 4.9](04-extensions-and-ecosystem.md)）的关系**：后者决定"截断多少"，spill 决定"截断掉的放哪"。
...
💡 **`session-checkpoint-policy` 只有 113 行**但很关键：决定"什么时候 flush"。压缩后是好时机（[08 § 8.39](08-execution-services.md)），turn 结束也是。
...
💡 **`web-app` bundle 里默认关闭**（[05 § 5.3](05-startup-and-cordis-runtime.md)）：
...
💡 **`dsh-base` patch 里三行相邻**（[05 § 5.11](05-startup-and-cordis-runtime.md)）：`typert`（= `dsh-typert-registry`）→ `typert-loader` → `typert-gateway`（= `dsh-api-gateway`）。
...
💡 **`@Remote` 有三个重载签名**（`:168`、`:177`、`:178`）——支持带/不带 `exportName` 参数的形式，与 cordis 的 `@Inject()`（[cordis 05 § 5.3](../hello-cordis/05-service-registration-and-dependency-resolution.md)）是同一风格。

## 9.33 `schemastery`：配置 schema

`vendor/schemastery`（3.18.1，[05 § 5.13](05-startup-and-cordis-runtime.md)）。

💡 **DSH 的插件 Config 都用 schemastery**（`z.object({...})`，如 [07 § 7.20](07-request-pipeline-llm-tools-and-prompts.md) 的 `SystemPrompt.Config`）。它实现了 Standard Schema 接口，所以 cordis 的 `resolveConfig`（[cordis 03 § 3.6](../hello-cordis/03-fiber-model.md)）能直接用。

💡 **`client-schema-form`（195 行）从 schema 生成表单**（[09 § 9.2](09-host-runtime-and-storage.md)）——**同一份 schema 三用**：运行时校验、类型推导、UI 生成。
...
| `XDG_*` 环境变量 | `BOOTSTRAP_PREFIXES` 之一（[05 § 5.8](05-startup-and-cordis-runtime.md)） |
...
> 路径缩写：`P/` = `packages/`，`TP/` = `packages/typert/protocol/src/`。行号对应快照 `fb2c4b9e`。
...
💡 **`dsh-base` patch 里三行相邻**（[05 § 5.11](05-startup-and-cordis-runtime.md)）：
...
💡 **`TypertLookupFailure<Failure = unknown>`（`:25`）带泛型**：查找失败时能携带结构化的失败原因，而不只是消息字符串。与 `LlmFailure`（[07 § 7.7](07-request-pipeline-llm-tools-and-prompts.md)）是同一思路。
...
`ctx.tools` 里同一个工具的 schema 有四种形态（[07 § 7.9](07-request-pipeline-llm-tools-and-prompts.md)）：
...
💡 **这解释了 `python/sdk` 的存在**（[04 篇](04-extensions-and-ecosystem.md)）——Code Mode 里模型能写 Python，就需要 Python 侧的运行时。

## 9.40 代码生成脚本一览

`scripts/` 里与类型/生成相关的（[02 § 2.8](02-codebase-map.md)）：
...
💡 **每个 `gen-*` 都有对应的 `verify-*`（或 `--check` 参数）**。根 `package.json` 的 `hygiene` 脚本串起来跑（[02 § 2.8](02-codebase-map.md)）。**生成物进版本库 + CI 校验一致性**是这类工具链的标准做法。

## 9.41 `*.generated.ts` 文件

已知的一个：`P/core/scope/src/scoped-events.generated.ts`（49 行，[03 § 3.6](03-capability-seams-and-services.md)）。
...
| 工具 | 配置 | 作用 |
|---|---|---|
| TypeScript | 5 份 tsconfig（[02 § 2.9](02-codebase-map.md)） | host / client 分离 |
...
💡 **`verify-node-next-types`** 针对 ESM + `NodeNext` 的 `.ts` 扩展名 import 问题（DSH 的 import 都写 `./x.ts`，见 vendor cordis 的 import 风格，[05 § 5.15](05-startup-and-cordis-runtime.md)）。

💡 **`doc-typecheck` + `doc-budgets.manifest.json`** 是很少见的实践：文档里的代码块**必须能编译**，且文档有**长度预算**。169055 行 markdown 靠工程手段维持质量（[02 § 2.8](02-codebase-map.md)）。

---

**上一篇** ← [08 执行侧服务：文件、Shell、沙箱、子代理、压缩](08-execution-services.md) ｜ **下一篇** → [10 测试与工程实践](10-testing-and-engineering.md)：测试占 45%、逐文件 100% 覆盖率门禁、双轨 CI。
