...
> **路径缩写**：`P/` = `packages/`，`AB/` = `packages/boot/app-boot/src/`，`V/` = `vendor/cordis/src/`；上游用 `C/` = cordis 仓库的 `packages/core/src/`。行号：DSH 侧对应 `47f94385`，cordis 侧对应 `8cc9e33f`。
>
> 🧭 **本篇导览**：5.1–5.12 启动、Profile 与 Bundle → 5.13–5.16 vendored cordis 分叉 → 5.17–5.20 Cordis 原语与 DSH Scope。
>
> 📎 **来源**：本篇合并自原 03 / 04 篇，章节已重新连续编号为 5.x。
...
2. **`structuredClone`** —— 深拷贝，避免 patch 处理时原地修改污染缓存的 patch 数据（cordis 的 `applyPatches` 是原地改的，见 [cordis 07 § 7.5](../hello-cordis/07-Loader与配置树.md)）。
...
**patch 可以只写 `disabled: true` 而不写 config**——因为 `disabled` 是 `EntryOptions` 的独立字段（[cordis 07 § 7.2](../hello-cordis/07-Loader与配置树.md)）。
...
💡 **三个设计点**：
1. **先 `ctx.fiber.dispose()`**，注释说明"根 fiber 卸载会按观察者隔离清理失败（cordis fiber.ts 加固），重复调用返回已 settle 的单次结果，所以这个 await 不会 reject 并替换 `cause`"。**这正是 [01 § 1.6](01-项目概览.md) 提到的 vendored cordis 的 `emitPluginDisposed` 加固在起作用。**
...
即：**group 行是"给 provider 和它的 consumer 一个共同的 isolate realm"的手段**（对应 [cordis 07 § 7.7](../hello-cordis/07-Loader与配置树.md)），而 workspace 外的 agent preset 无法按名字解析到 group 包——所以必须作为 builtin 注入。这直接关联 [03 篇](03-能力缝与服务全景.md) 的 seam 隔离机制。
...
💡 **区分"引导期变量"与"应用变量"的意义**：引导期变量（`DSH_*`、`XDG_*`、`DYLD_*`、`BASH_FUNC_*`）影响 dsh 自身怎么启动，不应该被 `.env` 文件覆盖或传给子进程；应用变量则相反。这是 [08 篇](08-执行侧服务-文件Shell沙箱子代理压缩.md)（子进程环境）与 [08 篇](08-执行侧服务-文件Shell沙箱子代理压缩.md)（沙箱环境）的前置。
...
💡 **`watchUserPatches` 让改 patch 文件即时生效**，配合 cordis include 的 `refresh()`（[cordis 07 § 7.5](../hello-cordis/07-Loader与配置树.md)）。对应测试 `AB/../tests/config-reload.spec.ts`（431 行）与 `user-patches.spec.ts`（422 行）。
...
即：**命令行参数通过一个普通 cordis 服务传给配置行**，配置里的 JS 表达式（[cordis 07 § 7.5](../hello-cordis/07-Loader与配置树.md) 的 `!!js` 标签）依赖该服务，因此**只在服务就绪后才求值**。`dsh --profile web --help` 不提供这个服务，所以不会真的绑定端口。
...
> 路径缩写：`P/` = `packages/`，`V/` = `vendor/cordis/src/`；上游用 `C/` = cordis 仓库的 `packages/core/src/`。行号：DSH 侧对应 `47f94385`，cordis 侧对应 `8cc9e33f`。
...
|---|---|---|---|
| `cordis` | `@deepseek-ai/cordis` | **4.0.1** | `cordis@4.0.0-rc.8` |
...
💡 **为什么 vendor 而不是直接依赖**：cordis 上游 README 明示 API 不稳定（[cordis 01 § 1.9](../hello-cordis/01-项目概览与设计哲学.md)）。DSH 要在这个基础上做产品，必须锁死一个可控的副本。同时 DSH 也在给它加文档与加固（§ 5.15）。
...
💡 **它用了上游标记为 `@deprecated` 的 `events.dispatch()`**（[cordis 06](../hello-cordis/06-事件系统与Waterfall.md) 里 `C/events.ts:84`）——因为它需要**拿到 callback 列表自己遍历**，而不是让 `emit` 一把跑完。这解释了那个 API 为什么还留着。

💡 **[05 § 5.5](05-启动与Cordis落地.md) 的 `boot()` 直接依赖这个加固**：它的注释写"Root-fiber disposal contains cleanup failures per observer (Cordis fiber.ts hardening) and a repeated call returns the settled single-shot result, so this await cannot reject and replace `cause`"。
...
**为什么需要**：DSH 的配置里大量使用 cordis 的 JS 表达式（`!!js` 标签，[cordis 07 § 7.5](../hello-cordis/07-Loader与配置树.md)），而表达式的求值依赖当时可用的服务。`web-app` patch 的注释说得很清楚：
...
> 📐 **配套可跑示例**：[`17-dispatch-modes`](../../dsh-example/17-dispatch-modes/index.ts) 在真实 `@deepseek-ai/cordis` 上把五种派发模式各跑一遍
> （含 `parallel` 的 `AggregateError`、`serial` 的截链、`waterfall` 的"最后一个实参就是内建默认"）；
> [`26-cordis-timer`](../../dsh-example/26-cordis-timer/run.ts) 演示 effect 化的定时器与 fiber 卸载时的拒绝语义。
...
- **可跑的最小示例**：dsh-example 里 26 个示例各演示一个扩展点 · 例如 26-cordis-timer 演示如何接入定时器 · 装的是真实发布的 npm 包

## 5.18 DSH 在 cordis 之上加的一层：`Scoped<T>` 与 `core/scope`

> 📐 **配套可跑示例**：[`11-tool-restrict`](../../dsh-example/11-tool-restrict/index.ts) 是 scope 的最小落点 ——
...
|---|---|---|
| `Context`（`ctx`） | 每个插件、每个 Agent（`agent.ctx`）都有自己的视图 | [03](03-能力缝与服务全景.md) |
| `Fiber` | 一行配置的一次运行；启动审计读它的 state | [05 § 5.9](05-启动与Cordis落地.md) |
| `effect()` | 全部注册（工具/prompt/监听/连接）的登记方式 | 各篇 |
| `Service` + `provide` / `inject` | 每个 `ctx.xxx` 能力 | [03](03-能力缝与服务全景.md) |
| `waterfall` | `tools/*`、`agent/pre-step`、`agent/request-error`、`llm/stream` | [07](07-请求管线-LLM工具与提示.md) [06](06-Agent循环与会话日志.md) [07](07-请求管线-LLM工具与提示.md) |
| `serial` | `agent/turn-stopping`（无 `next`） | [06](06-Agent循环与会话日志.md) |
| `emit` | `session/event` 广播 | [06](06-Agent循环与会话日志.md) |
| `isolate` realm | agent preset 里给 provider + consumer 一个 realm | [03](03-能力缝与服务全景.md) |
| `Group`（配置分组） | 把 provider 与 consumer 圈到一个 realm 的手段 | [05 § 5.6](05-启动与Cordis落地.md) |
| `include` + `applyPatches` | profile / bundle 分层 | [05 § 5.2](05-启动与Cordis落地.md) |
| `hmr` | 开发期插件热重载（web-app 里被 disable） | [05 § 5.11](05-启动与Cordis落地.md) |
| `timer` | 可逆定时器（`ctx.debounce` 等） | — |
| `logger` | 全仓日志 | — |
| Standard Schema（`Config`） | 配置校验；DSH 用 `schemastery`（vendored） | [09](09-宿主与运行面-Web网关编排存储类型.md) |

---

**上一篇** ← [04 扩展与生态](04-扩展与生态.md) ｜ **下一篇** → [06 Agent 循环与会话日志](06-Agent循环与会话日志.md)：496 行主循环逐行拆解 + append-only 会话日志的不变量。
