...
💡 **`standardDecoratorPlugin`（`vitest.config.ts:6`）** —— DSH 用**标准装饰器**（TC39 stage 3），不是 TS 的 legacy decorators。`@Inject()`（[cordis 05 § 5.3](../hello-cordis/05-服务注册与依赖解析.md)）与 `@Remote()`（[09 § 9.37](09-宿主与运行面-Web网关编排存储类型.md)）都是标准形式。
...
**测试必须走源码而非构建产物**，否则会出现"两份模块单例"——在 219 包的 monorepo 里这是真实风险（cordis 的服务注册依赖单例，[cordis 05 § 5.4](../hello-cordis/05-服务注册与依赖解析.md)）。
...
- **依赖 replayState**：assembler 的 replayState 就是为此设计 · 它让流式装配可以从录制数据重建 · 见 [07 篇 § 7.6](07-请求管线-LLM工具与提示.md)
...
框架本体：`P/runtime-diagnostics/invariants`（540 行，[04 § 4.8](04-扩展与生态.md)）。

💡 **invariant 与测试的关系**：测试验证"给定输入产生正确输出"，invariant 验证"运行时状态永不违反某条约束"。**二者互补**——invariant 能在生产环境捕获测试没覆盖的路径。

💡 **一个具体例子**（`P/fs/fs/src/invariant.ts:23-27`，[08 § 8.3](08-执行侧服务-文件Shell沙箱子代理压缩.md)）：只对三个 `fs/*` 事件名生效，`fs/observed` 有额外形状检查。
...
💡 **`demo:cordis` 的描述是"agent 修改自己的运行时"** —— 这是 `tool-cordis`（5984 行，[04 § 4.4](04-扩展与生态.md)）的演示。**自我修改是被当作正式能力演示的。**
...
| `scripts/gen-*` + `verify-*` | 生成物一致性（[09 § 9.40](09-宿主与运行面-Web网关编排存储类型.md)） |
...
**上一篇** ← [09 宿主与运行面：Web、网关、编排、存储、类型](09-宿主与运行面-Web网关编排存储类型.md) ｜ **下一篇** → [11 关键调用链速查](11-关键调用链速查.md)：四条核心链、事件全表、逐包符号表、症状路由。
