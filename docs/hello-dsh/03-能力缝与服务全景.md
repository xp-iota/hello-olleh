...
> **路径缩写**：`P/` = `packages/`，`SC/` = `packages/core/scope/src/`。行号对应快照 `47f94385`。
>
...
> 📐 **配套可跑示例**：[`05-llm-adapter`](../../dsh-example/05-llm-adapter/index.ts) 是三角色模型的最小实例 ——
...
详见 [08 篇](08-执行侧服务-文件Shell沙箱子代理压缩.md)。
...
💡 **`scopeTarget(this, scope)` 是事件派发的关键**。[07 § 7.21](07-请求管线-LLM工具与提示.md) 里 `assemble()` 调 waterfall 时传的就是它：

```ts
await this.ctx.waterfall(scopeTarget(this, scope), 'system-prompt/assemble', ...)
```

cordis 的 `waterfall(thisArg, name, ...)` 会用 `thisArg[Context.filter]` 过滤监听器（[cordis 06 § 6.2](../hello-cordis/06-事件系统与Waterfall.md)）——**`Scoped<T>` 载体携带了 scope 信息，从而实现"agent-scoped 监听器只收到那个 agent 的调用"**。
...
💡 **`ScopedLayers.effect(...)`（`SC/store.ts:226`）把"往某层插入一项"包成 cordis effect**。所以作用域化注册也是可逆的——[07 § 7.25](07-请求管线-LLM工具与提示.md) 里 `systemPrompt.section()` 就是这么实现的。

💡 **`scoped-events.generated.ts`（49 行）** 是脚本生成的——大概是把"哪些事件支持 scope 过滤"的清单生成出来，供运行时校验。改它要改生成器（[02 § 2.8](02-代码结构地图.md)）。
...
| `timeout-policy` | guard | — | `tools/execute`（[07 § 7.11](07-请求管线-LLM工具与提示.md)） |
| `gang-scheduling-*` | — | — | — |

## 3.8 全部 seam 一览

> 📐 **配套可跑示例**：下面这张表里的每个 seam 都有一个能真跑的最小 provider / consumer ——
> `ctx.llm` → [05](../../dsh-example/05-llm-adapter/index.ts)、`ctx.fs` / `ctx.subprocess` → [16](../../dsh-example/16-fs-shell-side-effects/index.ts)、
> `ctx.shell` → [24](../../dsh-example/24-shell-service/run.ts)、`ctx.sandbox` → [25](../../dsh-example/25-sandbox-seam/index.ts)、
> `ctx.compaction` → [14](../../dsh-example/14-compaction-provider/index.ts)、`ctx.subagents` → [15](../../dsh-example/15-subagent-delegation/index.ts)、
> `ctx.jobs` → [21](../../dsh-example/21-jobs-background/run.ts)、`ctx.goals` → [22](../../dsh-example/22-goal-lifecycle/run.ts)、
> `ctx.settings` → [23](../../dsh-example/23-settings-namespaces/index.ts)、`ctx.approval` → [13](../../dsh-example/13-approval-answerer/index.ts)。
...
| `persona` | 人格（对应 `PERSONA_SECTION`，[07 § 7.20](07-请求管线-LLM工具与提示.md)） |

💡 **agent preset 是"给一个会话不同能力集"的载体**。它本质上是一段 cordis 配置（含 group 行 + `isolate` 声明），[05 § 5.6](05-启动与Cordis落地.md) 提到的"workspace 外的 agent preset 无法按名字解析 group 包"就是这个场景。

---

**上一篇** ← [02 代码结构地图](02-代码结构地图.md) ｜ **下一篇** → [04 扩展与生态](04-扩展与生态.md)：Skill / hooks 桥接 / `tool-cordis` / MCP·ACP / Python SDK。
