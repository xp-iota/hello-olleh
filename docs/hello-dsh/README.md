...
完整命令与 12 个方向模块索引见 [`dsh-example/README.md`](../../dsh-example/README.md)。建议形成固定循环：**先跑 → 对照 `impl/*.ts` → 回读对应章节 → 改一个值再跑**。

### 示例工程结构与工具调用回路

下面两张图分别展示 12 个方向模块的组织方式，以及接入推理服务后一次工具调用的流程。
结构图的指标来自 graphify 建图；工具调用图中的 step 数与词数来自本地 SSE 桩测试。

![示例工程拓扑](diagrams/12-example-topology.svg)
...
- **依赖图谱**：584 节点、744 边、45 社区；`createHarness` 度数 66，是唯一枢纽；739 条边 `EXTRACTED`、5 条 `INFERRED`。
...
- **工具定义复用**：示例 01 注册的 `defineTool` 原样复用；provider 固定 `anthropic-compat`；schema 汇入提示由 `systemPrompt` 负责。
...
"模型"那一处是真实的 `LlmAdapter` 子类 `AnthropicCompatAdapter`，打真实推理服务（需 `LLM_API_KEY`）。

| 文档章节 | 配套示例 |
|---|---|
| [03 § 3.1](03-capability-seams-and-services.md) seam 三角色 | 03 inference-service-access |
...
| [07 § 7.2](07-request-pipeline-llm-tools-and-prompts.md) `LlmAdapter` · [§ 7.3](07-request-pipeline-llm-tools-and-prompts.md) `llm/stream` | 03 llm-stream · 20 stream-intercept |
