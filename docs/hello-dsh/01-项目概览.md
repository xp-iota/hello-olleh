...
配合 `0.1.0-rc.5` 与"无 tag"，说明这是**刚公开、快速迭代中**的项目。快照里最近几个 commit 就是 `release(dsh): 0.1.0-rc.5`、`build(release): publish the dsh family publicly`——公开发布刚完成。
...
顶层目录体积与布局见 [02 § 2.1](02-代码结构地图.md)。
...
测试口径：**766 个 spec / 254848 行，占代码 45%**（`packages/` + `apps/` 下的 `*.spec.ts(x)`；`scripts/` 下另有 45 个不计）。展开见 [10 § 10.1](10-测试与工程实践.md)。
...
详见 [06 篇](06-Agent循环与会话日志.md)（turn/step）与 [06 篇](06-Agent循环与会话日志.md)（事件日志）。
...
详见 [05 篇](05-启动与Cordis落地.md)、[03 篇](03-能力缝与服务全景.md)。

> 📐 **这三个概念都能立刻上手跑**：本仓库的 [配套示例工程](../../dsh-example/README.md) 有 26 个最小示例，
> 全部装在**真实发布的 `@deepseek-ai/*` npm 包**上（`0.1.1-rc.2`）——
> profile/bundle 看 [`cordis.yml`](../../dsh-example/cordis.yml)，
> seam 看 [05 llm-adapter](../../dsh-example/05-llm-adapter/index.ts) / [25 sandbox](../../dsh-example/25-sandbox-seam/index.ts)，
> 三类事件域看 [06 telemetry](../../dsh-example/06-agent-events-telemetry/index.ts) / [09 lifecycle](../../dsh-example/09-hooks-lifecycle-steering/index.ts) / [03 权限门](../../dsh-example/03-permission-gate/index.ts)。
...
|---|---|---|
| `@deepseek-ai/cordis` | **4.0.1** | `cordis@4.0.0-rc.8` |
...
💡 **含义**：[cordis 文档集](../hello-cordis/README.md) 分析的是**上游 4.0.0-rc.8**。机制、API、语义与 DSH 完全一致（有效代码差异 <3%），可以放心用来理解 DSH。但**行号不能直接套用**——vendor 版因为加了注释，行号整体后移。[05 篇](05-启动与Cordis落地.md) 给出两边的对照。
...
**返回** ← [索引](README.md) ｜ **下一篇** → [02 代码结构地图](02-代码结构地图.md)：`packages/` 下 219 个包怎么组织、六类命名后缀的语义、"想改 X 去哪"。
