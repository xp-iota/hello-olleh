...
> **路径缩写**：`P/` = `packages/`，`AL/` = `P/core/agent-loop/src/`，`AG/` = `P/core/agent/src/`，`S/` = `P/core/session/src/`，`T/` = `P/core/tools/src/`，`SP/` = `P/core/system-prompt/src/`，`SC/` = `P/core/scope/src/`，`AB/` = `P/boot/app-boot/src/`，`L/` = `P/llm/llm/src/`，`V/` = `vendor/cordis/src/`。行号对应快照 `47f94385`。
>
...
💡 **日志一行未删** —— 这是 append-only + 表面机制的核心红利（[06 § 6.15](06-Agent循环与会话日志.md)、[08 § 8.34](08-执行侧服务-文件Shell沙箱子代理压缩.md)）。
...
见 [05 § 5.16](05-启动与Cordis落地.md) 完整表。要点：
...
vendor 独有（三处加固，[05 § 5.15](05-启动与Cordis落地.md)）：`effectInertia`（`V/fiber.ts:112`）、`runDisposable`（`:114`）、`emitPluginDisposed`（`:120`）、`_config`（`:192`）、`_resolveConfig`（`:641`）。
...
| 症状 | 先看哪篇 |
|---|---|
| 不知道从哪读起 | [01](01-项目概览.md)、[02](02-代码结构地图.md) |
| 不懂 cordis | 先读 [cordis 文档集](../hello-cordis/README.md)（03/04/05 必读） |
| 启动失败 / 想看实际插件树 | [05](05-启动与Cordis落地.md)（`--dump-config`） |
| patch 不生效 | [05 § 5.3](05-启动与Cordis落地.md)（整体替换语义） |
| cordis 行号对不上 | [05 § 5.16](05-启动与Cordis落地.md)（vendor 对照表） |
| turn/step 行为疑问 | [06](06-Agent循环与会话日志.md) |
| 想拦截模型看到的内容 | [06 § 6.5](06-Agent循环与会话日志.md)（`agent/pre-step`） |
| 事件加了模型看不到 | [06 § 6.13](06-Agent循环与会话日志.md)（只有 3 种上表面） |
| 会话历史/fork/持久化 | [06](06-Agent循环与会话日志.md)、[09](09-宿主与运行面-Web网关编排存储类型.md) |
| 接新模型 | [07 § 7.2](07-请求管线-LLM工具与提示.md)（只需 `stream()`） |
| 工具不执行 / 顺序不对 | [07](07-请求管线-LLM工具与提示.md) |
| Code Mode | [07 § 7.15](07-请求管线-LLM工具与提示.md) |
| 提示词组装 | [07](07-请求管线-LLM工具与提示.md) |
| 想换某个能力的实现 | [03](03-能力缝与服务全景.md)（seam 全表） |
| 按 agent 隔离注册 | [03 § 3.5](03-能力缝与服务全景.md)（两级隔离） |
| 文件读写/编辑冲突 | [08](08-执行侧服务-文件Shell沙箱子代理压缩.md) |
| 命令执行 / 远端沙箱 | [08](08-执行侧服务-文件Shell沙箱子代理压缩.md) |
| 沙箱不可用 / 审批 | [08](08-执行侧服务-文件Shell沙箱子代理压缩.md) |
| subagent / 委派 | [08](08-执行侧服务-文件Shell沙箱子代理压缩.md) |
| 上下文太长 | [08](08-执行侧服务-文件Shell沙箱子代理压缩.md)、[09 § 9.29](09-宿主与运行面-Web网关编排存储类型.md)（spill） |
| 加能力选哪条路 | [04 § 4.1](04-扩展与生态.md) |
| MCP / ACP | [04](04-扩展与生态.md) |
| 前端 / Chat 节点 | [09](09-宿主与运行面-Web网关编排存储类型.md) |
| API / 网关 / 端口 | [09](09-宿主与运行面-Web网关编排存储类型.md) |
| 后台任务 / 定时 / 目标 | [09](09-宿主与运行面-Web网关编排存储类型.md)（选择表） |
| 存储选哪种 | [09 § 9.27](09-宿主与运行面-Web网关编排存储类型.md) |
| RPC / schema / 代码生成 | [09](09-宿主与运行面-Web网关编排存储类型.md) |
| Python 驱动 | [04](04-扩展与生态.md) |
| 覆盖率 / 测试 / CI | [10](10-测试与工程实践.md) |
| 想查某符号在哪 | 本篇 § 11.6 |

---

**上一篇** ← [10 测试与工程实践](10-测试与工程实践.md) ｜ **返回** → [索引](README.md) ｜ **动手** → [配套示例工程](../../dsh-example/README.md)
