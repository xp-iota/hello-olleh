---
layout: content
title: "08 执行侧服务：文件、Shell、沙箱、子代理、压缩"
parent_url: /docs/hello-deepseek-harness/
---

# 08 执行侧服务：文件、Shell、沙箱、子代理、压缩
...
> **路径缩写**：`P/` = `packages/`，`F/` = `packages/fs/fs/src/`，`SH/` = `packages/shell/shell/src/`，`SP/` = `packages/subprocess/subprocess/src/`，`TM/` = `packages/terminal/terminal/src/`，`SB/` = `packages/sandbox/sandbox/src/`，`AG/` = `packages/core/agent/src/`，`CP/` = `packages/compaction/compaction/src/`。行号对应快照 `47f94385`。
>
> 🧭 **本篇导览**：8.1–8.9 文件系统 → 8.10–8.16 子进程与 Shell → 8.17–8.25 沙箱、审批与防护 → 8.26–8.32 子代理 → 8.33–8.39 上下文压缩。
>
> 📎 **来源**：本篇合并自原 11 / 12 / 13 / 14 / 15 篇，章节已重新连续编号为 8.x。
...
💡 **定义包 503 行，三个 consumer 3590 行**。这是健康的 seam 比例（[03 § 3.1](03-能力缝与服务全景.md)）。

## 8.2 `FileSystem` 抽象类：13 个抽象方法

> 📐 **配套可跑示例**：[`16-fs-shell-side-effects`](../../dsh-example/16-fs-shell-side-effects/index.ts) 的 `write_note` 工具只用
> `resolve` → `writeText` → `processPath` 三个口，就在真实 `LocalFileSystem`（`@deepseek-ai/dsh-fs-local`）上落了盘。


`F/index.ts:86`（`extends Service`）。**全部是 abstract**，没有默认实现——与 `LlmAdapter`（只有 1 个必需，[07 § 7.2](07-请求管线-LLM工具与提示.md)）形成鲜明对比。
...
💡 **`ctx.workspace` 大概管"工作区根目录、项目边界"**。它与 `fs` 的分工：`workspace` 定义"哪里是项目"，`fs` 提供"怎么读写"。`web-app` patch 里的 persona 用 `{% raw %}{{cwd}}{% endraw %}` 变量（[07 § 7.20](07-请求管线-LLM工具与提示.md)）应该来自这里。前端有 `ui-workspace` 与 `ui-directory-picker-*` 子包。

---

> 📎 **以下承接原「12 Shell 终端与子进程」。**
> 本篇讲执行世界的三层：`ctx.subprocess`（最底层进程 spawn）、`ctx.shell`（一次性命令执行）、`ctx.terminals`（持久 PTY 会话）。以及为什么要分三层。
> 路径缩写：`P/` = `packages/`，`SH/` = `packages/shell/shell/src/`，`SP/` = `packages/subprocess/subprocess/src/`，`TM/` = `packages/terminal/terminal/src/`。行号对应快照 `47f94385`。
...
> 📐 **配套可跑示例**：[`24-shell-service`](../../dsh-example/24-shell-service/run.ts) 把三个方法都跑了一遍 ——
...
💡 **PowerShell 是一等公民**（`pwsh-local` 472 行 > `bash-local` 363 行）。这说明 DSH 认真支持 Windows——配合 `sandbox-windows-acl`（2530 行，[08 篇](08-执行侧服务-文件Shell沙箱子代理压缩.md)）。
...
💡 **`hasOwnerActivity(owner)`（`:231`）的用途**：在 turn 结束前检查"这个 agent 还有终端在活动吗"。如果有，可能需要延续 turn 或提醒模型（对应 `agent/turn-stopping`，[06 § 6.4](06-Agent循环与会话日志.md)）。

💡 **`registerBackend` 而非 provider 模式**：终端后端与 `ctx.terminals` 是"多后端共存"的关系（`listBackends()` 返回数组），而 `ctx.fs` 是"单 provider"。**这是 seam 的另一种形态**（[03 § 3.8](03-能力缝与服务全景.md) 提到 `ctx.sessionTitle` 是"唯一 provider"，这里是"多后端"）。
...
💡 **用 worker thread 而非子进程**：Code Mode（[07 § 7.15](07-请求管线-LLM工具与提示.md)）里模型写的代码要调 DSH 的工具，跑在 worker thread 里能共享内存中的工具注册表，比子进程 + IPC 简单得多。代价是隔离性弱（worker 崩溃可能影响主进程）。

## 8.16 环境变量的传递

关联 [05 § 5.8](05-启动与Cordis落地.md) 的 `BOOTSTRAP_PREFIXES = ['DSH_', 'XDG_', 'DYLD_', 'BASH_FUNC_']`：
...
> 路径缩写：`P/` = `packages/`，`SB/` = `packages/sandbox/sandbox/src/`。行号对应快照 `47f94385`。
...
> 📐 **配套可跑示例**：[`25-sandbox-seam`](../../dsh-example/25-sandbox-seam/index.ts) 提供两个后端演示两端 ——
...
💡 **"caller spawns the returned argv"是刻意的设计**（对照 [03 § 3.8](03-能力缝与服务全景.md)：`docs/architecture.md` 说 "consumers wrap argv before spawning"）。沙箱不是拦截器 —— **是否沙箱化在调用点显式可见**，代码审查时能直接看到哪些 spawn 没走 confine。
...
💡 **独立 LICENSE 文件**（`native/landlock-run/LICENSE`）说明它是可独立分发的组件，不只是内部工具。它在 `pnpm-workspace.yaml` 里也是独立的 workspace 成员（[02 § 2.1](02-代码结构地图.md)）。
...
> 📐 **配套可跑示例**：[`13-approval-answerer`](../../dsh-example/13-approval-answerer/index.ts) 把四值 outcome 的三种局面都跑了一遍。
...
| `timeout-policy` | 111 | 超时（`tools/execute` wrapper，[07 § 7.11](07-请求管线-LLM工具与提示.md)） |
...
💡 **凭据与 LLM 分离**：`llm` 只负责校验拿到的 key 可用（`assertUsableApiKey`，[07 § 7.8](07-请求管线-LLM工具与提示.md)），存取由 `ctx.credentials` 管。这样换存储后端（本地文件 → 系统钥匙串 → 远端 vault）不影响 llm 层。

## 8.25 `.github/workflows/sandbox.yml`

CI 里有专门的 sandbox workflow（[02 § 2.9](02-代码结构地图.md) 的 15 个 workflow 之一），另有 `landlock-run.yml` 与 `landlock-run-release.yml`。
...
> 路径缩写：`P/` = `packages/`，`AG/` = `packages/core/agent/src/`。行号对应快照 `47f94385`。
...
- `subagent-fork-in-process`（124）：调 `ctx.sessions.fork(...)`（[06 § 6.19](06-Agent循环与会话日志.md)）再建 agent

**因为 append-only 日志天然支持 fork，"分叉一个子 agent"只要 124 行。**

## 8.27 六种驱动的谱系

> 📐 **配套可跑示例**：[`15-subagent-delegation`](../../dsh-example/15-subagent-delegation/index.ts) 写了第七种（一个离线的
...
💡 **`inject` 语义的呼应**（[06 § 6.2](06-Agent循环与会话日志.md)）：报告用 `deferContext`（追加到 `next-step`，不唤醒），所以**父 agent 不会因为收到报告就白跑一个 step**——它等自己本来的下一步。
...
- **goal**：在**同一个会话**里管理一个目标，靠 `goal-round-driver` 在 `agent/turn-stopping` 里延续 turn（[06 § 6.4](06-Agent循环与会话日志.md) ④）
...
> 路径缩写：`P/` = `packages/`，`CP/` = `packages/compaction/compaction/src/`。行号对应快照 `47f94385`。
...
💡 **`command-compact` 是 `command-*` 而非 `tool-*`**——压缩由**人**触发，不经模型 turn（[08 § 8.21](08-执行侧服务-文件Shell沙箱子代理压缩.md)）。当然自动压缩走另一条路（§ 8.34）。

## 8.33 `CompactionEngine`：三个抽象方法

> 📐 **配套可跑示例**：[`14-compaction-provider`](../../dsh-example/14-compaction-provider/index.ts) 是这个抽象类的最小 provider，
...
> 📐 **配套可跑示例**：[`14-compaction-provider`](../../dsh-example/14-compaction-provider/run.ts) 实测把 12 个 surface 节点折成 4 个
...
这是 DSH 的一个关键设计，全靠 [06 § 6.15](06-Agent循环与会话日志.md) 的表面机制。
...
💡 **`compactIfNeeded` 需要知道"当前用了多少 token"** —— 这个数据由 `token-meter` 提供（它监听 `llm/stream`，[07 § 7.3](07-请求管线-LLM工具与提示.md)）。压缩的触发阈值就是基于它的计量。
...
**上一篇** ← [07 请求管线：LLM、工具与 SystemPrompt](07-请求管线-LLM工具与提示.md) ｜ **下一篇** → [09 宿主与运行面：Web、网关、编排、存储、类型](09-宿主与运行面-Web网关编排存储类型.md)：前端、网关、五种编排、四条存储、类型系统。
