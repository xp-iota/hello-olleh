---
layout: content
title: "07 请求管线：LLM、工具与 SystemPrompt"
parent_url: /docs/hello-deepseek-harness/
---

# 07 请求管线：LLM、工具与 SystemPrompt
...
> **路径缩写**：`P/` = `packages/`，`L/` = `packages/llm/llm/src/`，`T/` = `packages/core/tools/src/`，`AL/` = `packages/core/agent-loop/src/`，`SP/` = `packages/core/system-prompt/src/`。行号对应快照 `47f94385`。
>
...
💡 **waterfall 返回 `AsyncIterable` 而非 Promise**——监听器可以包装流（逐 chunk 加工）而不必等整个流结束。这是“流式可拦截”的关键（对照 [cordis 06 § 6.3](../hello-cordis/06-事件系统与Waterfall.md)，waterfall 的返回类型由事件签名决定）。

💡 **waterfall 的通用原理不在本篇展开**：两侧契约、执行语义与踩坑集中在 [cordis 06 § 6.3](../hello-cordis/06-事件系统与Waterfall.md)；本篇的三处 waterfall（§ 7.3 / § 7.11 / § 7.26）只讲各条缝自己的约定。
...
💡 **`prepareCall` 的存在解释了 [06 § 6.6](06-Agent循环与会话日志.md) 里那行代码**：
...
`L/assembler.ts`（164 行）。用法在 [06 § 6.6](06-Agent循环与会话日志.md)：
...
💡 **签名带 `pkg` 与 `ref` 参数**——出错时能报出"是哪个包、哪个引用"的 key 不可用。这类"错误信息里带来源"的设计在 DSH 里很常见（对照 [05 § 5.5](05-启动与Cordis落地.md) 的 `binName` 参数）。
...
> 路径缩写：`P/` = `packages/`，`T/` = `packages/core/tools/src/`，`AL/` = `packages/core/agent-loop/src/`。行号对应快照 `47f94385`。
...
💡 **同时生成 TS 与 Python 类型**（`ts-types.ts` + `py-types.ts` 共 1111 行）是为 Code Mode 服务的：模型要写代码调工具，就需要工具的类型定义。这也解释了 `python/sdk` 的存在（[04 篇](04-扩展与生态.md)）。
...
即追加到 `next-step` 队列尾部。`concludeTurn()` 则让 `executeToolCalls` 返回 `concluded: true`，使 `step()` 返回 `{ kind: 'completed' }`（[06 § 6.6](06-Agent循环与会话日志.md)）。
...
💡 **`schemas(scope?)` 是"每个 agent 看到不同工具集"的实现点**。`scope` 是 `ScopeKey`；不传则返回全局。这与 [07 篇](07-请求管线-LLM工具与提示.md) 的 prompt 组装直接相连——**工具 schema 是 system prompt 的一部分**。
...
全仓 `tool-*` 包共 **21 个**（[02 § 2.2](02-代码结构地图.md)）：
...
💡 **`tool-*` 包本身通常很小**——它们只是 `ctx.tools.register(...)` 的一层壳，真正的能力在同组的 provider 里（[03 篇](03-能力缝与服务全景.md)）。
...
> 路径缩写：`P/` = `packages/`，`SP/` = `packages/core/system-prompt/src/`。行号对应快照 `47f94385`。
...
💡 **变量插值发生在 `renderPrompt`**（`SP/index.ts:65` 注释："interpolated later, by `renderPrompt`"）。所以 `{% raw %}{{model}}{% endraw %}` / `{% raw %}{{cwd}}{% endraw %}` 这类占位符在 section 注册时是原样保存的，渲染时才替换。
...
💡 **`ScopedLayers` 来自 `P/core/scope`**（561 行，见 [03 篇](03-能力缝与服务全景.md)）。同一个原语被 `tools`、`system-prompt` 等多个 core 包复用——**"按 agent 作用域分层注册"是 DSH 的通用模式**。
...
`label` 会出现在 cordis 的 `EffectMeta` 里（[cordis 03 § 3.2](../hello-cordis/03-Fiber模型.md)），调试时能看到"这个 effect 是谁登记的"。
...
💡 **`this` 是 `Scoped<SystemPrompt>`**，与 `tools/*` 一致（[07 § 7.11](07-请求管线-LLM工具与提示.md)）——按 agent 作用域过滤。
...
💡 **`agent-instructions` 很关键**：它把项目里的 `AGENTS.md` / `CLAUDE.md` 读进提示词。DSH 自己的仓库就有这两个文件（149 行，内容相同，[02 § 2.9](02-代码结构地图.md)）——**DSH 用自己开发自己**。

💡 **`tmux-context` 的存在说明 DSH 设计时考虑了终端多路复用场景**——agent 能知道自己在哪个 tmux pane 里。

---

**上一篇** ← [06 Agent 循环与会话日志](06-Agent循环与会话日志.md) ｜ **下一篇** → [08 执行侧服务：文件、Shell、沙箱、子代理、压缩](08-执行侧服务-文件Shell沙箱子代理压缩.md)：agent 真正"动手"的那几个 seam 与它们的 fail-closed 边界。
