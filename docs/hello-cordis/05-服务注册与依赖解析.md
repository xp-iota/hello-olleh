...
> 📐 **配套可跑示例**：[`05-llm-adapter`](../../dsh-example/05-llm-adapter/README.md) 是对象插件的真实最小样本：导出 `inject = ['llm']` 与 `apply(ctx, config)`，再通过 effect-based 的 `ctx.llm.registerAdapter()` 注册 Provider。在 `dsh-example/` 运行 `npm run 05`，可观察依赖就绪后插件启动、服务注册与 Consumer 解析；`npm run 05:minimax` 再用同一个消费循环切到真实 `minimax-m3` 路由。其中 `ctx.llm` 是 DSH 建在 Cordis Service 机制上的领域服务。
...
💡 **`filter` 参数支持定制**。loader 的 isolate 切换逻辑传了一个自定义 filter（`L/config/isolate.ts:136-140`），只通知那些"跨越了 realm 边界"的 Fiber，避免误伤（见 [07 § 7.7](07-Loader与配置树.md)）。
...
| `symbols.tracker` | `Service.tracker` | 11 | traceable 配置（见 [04 § 4.6](04-Context与Reflect代理.md)） |
...
**上一篇** ← [04 Context 与 Reflect 代理](04-Context与Reflect代理.md) ｜ **下一篇** → [06 事件系统与 Waterfall](06-事件系统与Waterfall.md)：5 种派发模式、8 个 internal 事件、以及 waterfall 中间件链。
