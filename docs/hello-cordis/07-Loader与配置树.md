...
> **路径缩写**：`L/` = `packages/loader/src/`，`I/` = `packages/include/src/`，`C/` = `packages/core/src/`。行号对应快照 `8cc9e33f`。
>
...
`include` 插件把一个文件变成 EntryTree。**它的 `applyPatches` 是 DSH profile/bundle 系统的直接地基**（见 [DSH 05 篇](../hello-dsh/05-启动与Cordis落地.md)）。
...
**上一篇** ← [06 事件系统与 Waterfall](06-事件系统与Waterfall.md) ｜ **下一篇** → [08 HMR 热重载](08-HMR热重载.md)：模块图上的 accepted/declined 传播、Node 22/24 的缓存差异、以及失败回滚。
