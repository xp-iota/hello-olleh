...
title: 能力缝的三角色
description: DSH 的每个可替换能力都由 Definition、Provider、Consumer 三个角色构成；示例工程刻意留空两个缝来演示这一点。
tags: [okf, dsh-example, seam, extensibility]
status: stable
sources:
  - id: harness-ts
    resource: "dsh-example/runtime/harness.ts"
    title: 装配清单与「刻意不装载」的注释
  - id: m03-provider
    resource: "dsh-example/M03-inference-service-access/steps/01-llm-adapter.ts"
    title: M03 用 mock adapter 提供 LLM seam
  - id: m02-compaction
    resource: "dsh-example/M02-context-assembly-economics/steps/03-compaction-provider.ts"
    title: M02 的 CompactionEngine provider
  - id: m07-sandbox
    resource: "dsh-example/M07-execution-backends/steps/03-sandbox-seam.ts"
    title: M07 的 SandboxProvider provider
  - id: dsh-03
    resource: "docs/hello-dsh/03-能力缝与服务全景.md"
    title: 能力缝与服务全景分析
...
| Provider | 契约的具体实现，注册进某条路由 | `MockAdapter`、`MinimaxAnthropicAdapter` |
| Consumer | 通过服务门面使用能力，不感知具体实现 | `agent-loop` 调 `ctx.llm` |

判断「这是不是一条缝」的实操标准：**能不能在不改 Consumer 的前提下换掉 Provider**。
LLM 能力满足这一点，所以 [换成真实 MiniMax](provider-swap.md) 不需要改任何示例代码。

## 示例工程如何演示这一点

`createHarness` 装了 18 个真实服务，却**刻意不装**两个缝的后端：

| 留空的缝 | 真实产品里的后端 | 由谁在示例里补上 |
|:---------|:-----------------|:-----------------|
| `CompactionEngine` | `@deepseek-ai/dsh-compaction-basic` | M02 `steps/03-compaction-provider.ts` |
| `SandboxProvider` | `@deepseek-ai/dsh-sandbox-local` | M07 `steps/03-sandbox-seam.ts` |
...
- [装配枢纽 createHarness](assembly-hub.md)
- [推理服务可替换](provider-swap.md)
- 长文分析：[03 能力缝与服务全景](../../hello-dsh/03-能力缝与服务全景.md)
